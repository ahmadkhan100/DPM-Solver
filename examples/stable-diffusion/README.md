...
**DPM-Solver can greatly accelerate the text-to-image sampling by stable-diffusion**.

We provide a reference sampling script by combining DPM-Solver with stable diffusion. The script is based on the official sampling script by stable diffusion, which incorporates
...
By default, this uses a guidance scale of `--scale 7.5` and renders images of size 512x512 in **only 25 steps**. We empirically find that the **25-step DPM-Solver can even generate slightly better quality images than the 50-step PLMS solver**.
